...
[ ] 검색 기능 구현 (Elasticsearch 도입 고려)

[ ] 상품 데이터 모델 확장 (매운맛, 할랄, 영양 정보 등)

[ ] 재고 및 유통기한 관리

[ ] 실시간 재고 차감 로직 구현

[ ] 유통기한별 상품 상태 관리 (신선, 임박 등) 스케줄러 개발
...
[ ] 실시간 배송 추적을 위한 외부 API 연동

[ ] 다국어 지원 확장

[ ] 5개 국어 지원을 위한 데이터베이스 구조 변경 (e.g., product_translations 테이블)
...
[ ] 상품 상세 페이지에 레시피, 영양 정보 등 탭(Tab) UI 구현

[ ] 다국어/다통화 지원 완성

[ ] 영어, 러시아어, 아랍어 언어팩 추가

[ ] RTL(Right-to-Left) UI 지원 (아랍어)

[ ] 사용자 설정에 따른 통화 표시 변경 기능

[ ] 결제 옵션 확장

[ ] 할부 결제 옵션 UI 추가

[ ] Papara 등 추가 결제 수단 선택 UI 구현
...
[ ] 상품 리뷰 CRUD API (별점, 텍스트, 이미지 포함)

[ ] 레시피 공유 게시판 CRUD API

[ ] 개인화 시스템

[ ] AI 기반 상품 추천 엔진 연동 API

[ ] 개인별 유통기한 알림 발송 시스템 (Push/Email)

[ ] 성능 최적화

[ ] 주요 API 응답 캐싱 처리 (Redis 도입)

[ ] 데이터베이스 쿼리 최적화 (인덱싱, Slow Query 분석)
...
[ ] 상품 상세 페이지 내 리뷰 섹션 구현

[ ] 레시피 게시판 페이지 개발

[ ] 개인화 UI

[ ] 메인 페이지 및 상품 페이지에 개인화 추천 상품 영역 추가
